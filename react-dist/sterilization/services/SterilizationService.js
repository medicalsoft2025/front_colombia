...
import { statuses } from "../consts/index.js";
// Helper functions for mapping snake_case from API to camelCase for UI
const mapPackageDto = data => ({
  id: data.id,
  name: data.name,
  prefix: data.prefix,
  content: data.content
});
const mapLoadDto = data => ({
  id: data.id,
...
export class SterilizationService extends BaseApiService {
  ...
  async updateLoadStatus(id, status) {
    await this.httpClient.put(`medical/sterilizations/loads/${id}/status`, {
      status
    });
  }
  async getLoadHistory(loadId) {
