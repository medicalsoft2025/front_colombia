...
export const SpecialtyPatientViewConfigForm = (
    props: SpecialtyPatientViewConfigFormProps,
) => {
    const { onSave, specialtyId, formId } = props;

    const { updateSpecialtyPatientViewConfig, toast } =
    ...
    const {
        ...
        formState: { errors },
        setValue,
    } = useForm<SpecialtyPatientViewConfigFormInputs>({
        defaultValues: {
            visible_cards: [],
        },
    });
    ...
    const onSubmit = async (data: SpecialtyPatientViewConfigFormInputs) => {
        try {
            ...
            const response = await updateSpecialtyPatientViewConfig(
                specialtyId,
                formData,
            );
            onSave?.();
    ...
    const getFormErrorMessage = (
        name: keyof SpecialtyPatientViewConfigFormInputs,
    ) => {
        return (
            errors[name] &&
    ...
    async function loadCards() {
        ...
        const cards = [
            ...
            { value: "medicamentos-actuales", label: "Medicamentos actuales" },
            ...
            { value: "historial-familiar", label: "Antecedentes familiares" },
        ...
        if (tenantConfig?.is_migration) {
            cards.push({ value: "consulta-antigua", label: "Consultas medicas 1.0" });
        }
        setCards(cards);
    }
    ...
    useEffect(() => {
        if (specialtyId) {
            const asyncScope = async () => {
                ...
                setValue(
                    "visible_cards",
                    response.patient_view_config_json.visible_cards,
                );
            };
                                    ...
                                    render={({ field }) => (
                                        <>
                                            ...
                                            <small className="text-muted">
                                                Seleccione las cards que estarán visibles en la vista
                                                del paciente
                                            </small>
                                        </>
                                    )}
