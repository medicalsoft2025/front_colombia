...
import {
  DisabilityForm,
  DisabilityFormInputs,
} from "../disabilities/form/DisabilityForm";
import {
  Remission,
  remissionsForm as RemissionsForm,
} from "../remissions/RemissionsForm";
import PrescriptionForm, {
  PrescriptionFormInputs,
} from "../prescriptions/components/PrescriptionForm";
...
import {
  ...
  OptometryPrescriptionFormRef,
} from "../prescriptions/components/OptometryPrescriptionForm.js";
...
export const FinishClinicalRecordForm: React.FC<FinishClinicalRecordFormProps> =
  forwardRef((props, ref: Ref<FinishClinicalRecordFormRef>) => {
    ...
    const [finalClinicalRecordType, setFinalClinicalRecordType] =
      useState<string>(clinicalRecordType);
    const [finalPatientId, setFinalPatientId] = useState<string>(patientId);
    const [finalAppointmentId, setFinalAppointmentId] =
      useState<string>(appointmentId);
    const [finalSpecialtyName, setFinalSpecialtyName] =
      useState<string>(specialtyName);

    const { control, setValue, getValues, watch } =
      useForm<FinishClinicalRecordFormInputs>({
        defaultValues: {
          diagnosis: null,
          diagnoses: [],
          treatment_plan: null,
    ...
    const { cie11Codes, loadCie11Codes, cie11Code, setCie11Code } =
      useSpecialty();
    const { getClinicalRecord, clinicalRecord: clinicalRecordForUpdate } =
      useClinicalRecord();
    ...
    const [initialSelectedExamTypes, setInitialSelectedExamTypes] = useState<
      string[]
    >([]);
    const [initialDisabilityFormData, setInitialDisabilityFormData] = useState<
      DisabilityFormInputs | undefined
    >(undefined);
    const [initialRemissionData, setInitialRemissionData] = useState<
      Remission | undefined
    >(undefined);
    const [initialPrescriptionData, setInitialPrescriptionData] = useState<
      PrescriptionFormInputs | undefined
    >(undefined);
    const [loadLastPrescriptionCheck, setLoadLastPrescriptionCheck] =
      useState<boolean>(false);

    const [clinicalRecordTypeId, setClinicalRecordTypeId] = useState<string>(
      props.clinicalRecordTypeId || "",
    );
    ...
    const [prescriptionsActive, setPrescriptionsActive] =
      useState<boolean>(false);
    const [optometryActive, setOptometryActive] = useState<boolean>(false);
    ...
    const [attentionTypesRips, setAttentionTypesRips] = useState<any[]>([]);
    const [careModesRips, setCareModesRips] = useState<any[]>([]);
    const [serviceGroupsRips, setServiceGroupsRips] = useState<any[]>([]);
    const [serviceCodesRips, setServiceCodesRips] = useState<any[]>([]);
    const [consultationPurposesRips, setConsultationPurposesRips] = useState<
      any[]
    >([]);
    const [externalCausesRips, setExternalCausesRips] = useState<any[]>([]);
    ...
    const getRecipeTab = () => {
      if (
        ["historiaOptometria", "historiaOptometriaD"]
          .map((item) =>
            item
              .normalize("NFD")
              .replace(/[\u0300-\u036f]/g, "")
    ...
    const { lastPatientPrescription, loadLastPatientPrescription } =
    ...
    const onPackageChange = (pkg: any) => {
      ...
      const packageExamTypes = pkg.package_items.filter(
        (item: any) => item.item_type == "App\\Models\\Examen",
      );
      const packageExamTypeIds = packageExamTypes.map(
      ...
      const packageDisability = pkg.package_items.find(
        (item: any) => item.item_type == "App\\Models\\Incapacidad",
      );
      if (packageDisability) {
        ...
        setInitialDisabilityFormData({
          ...
          start_date: new Date(),
          end_date: addDaysToDate(
            new Date(),
            packageDisability.prescription.days_incapacity,
          ),
      ...
      const packageRemission = pkg.package_items.find(
        (item: any) => item.item_type == "App\\Models\\Remision",
      );
      if (packageRemission) {
        ...
        setInitialRemissionData({
          ...
          receiver_user_specialty_id:
            packageRemission.prescription.specialty_id,
          note: packageRemission.prescription.reason,
        });
      }

      const packagePrescriptions = pkg.package_items.filter(
        (item: any) => item.item_type == "App\\Models\\medicamento",
      );
      if (packagePrescriptions.length > 0) {
        ...
        setInitialPrescriptionData({
          ...
          is_active: true,
          medicines: [
            ...packagePrescriptions.map((item: any) => ({
              ...
              take_every_hours:
                +item.prescription.medication_frequency?.split(" ")[0] || 0,
              showQuantity: false,
              showTimeField: false,
            })),
    ...
    useEffect(() => {
      if (clinicalRecordForUpdate) {
        setFinalClinicalRecordType(
          clinicalRecordForUpdate.clinical_record_type.key_,
        );
        setFinalSpecialtyName(
          clinicalRecordForUpdate.created_by_user.specialty.name,
        );
        ...
        setValue(
          "diagnoses",
          clinicalRecordForUpdate.clinical_record_diagnoses.map(
            (diagnosis: any) => {
              return {
        ...
        const examRecipesDetails = clinicalRecordForUpdate.exam_recipes
          .map((examRecipe: any) =>
        ...
        if (clinicalRecordForUpdate.patient_disabilities.length > 0) {
          ...
          setInitialDisabilityFormData({
            user_id: 0,
            days_disability: daysBetweenDates(
              stringToDate(disability.start_date),
              stringToDate(disability.end_date),
            ),
        ...
        const prescriptionDetails =
          clinicalRecordForUpdate.recipes[0].recipe_items;
        ...
        if (prescriptionDetails.length > 0) {
          ...
          setInitialPrescriptionData({
            ...
            is_active: true,
            medicines: [
              ...prescriptionDetails.map((item: any) => ({
    ...
    async function loadResourcesToRips() {
      const attentionTypes =
        await resourcesMedicalService.getConsultationTypesRips();
      const careModes = await resourcesMedicalService.getCareModesRips();
      const serviceGroups =
        await resourcesMedicalService.getServiceGroupsRips();
      const serviceCodes = await resourcesMedicalService.getServiceCodesRips();
      const consultationPurposes =
        await resourcesMedicalService.getConsultationPurposesRips();
      const externalCauses =
        await resourcesMedicalService.getExternalCausesRips();
    ...
    const handleLoadLastPrescriptionChange = async (e: boolean) => {
      ...
      if (e && selectedPackage) {
        ...
        const newMedicines = [
          ...(initialPrescriptionData?.medicines || []),
          ...lastPrescription.recipe_items,
        ];
        setInitialPrescriptionData({
          user_id: 0,
          patient_id: 0,
        ...
        setInitialPrescriptionData({
          ...
          patient_id: 0,
          is_active: true,
          medicines: selectedPackage.package_items
            .filter((item: any) => item.item_type == "App\\Models\\medicamento")
            .map((item: any) => ({
              medication: item.prescription.medication,
              concentration: item.prescription.concentration, //
              duration: item.prescription.duration_days, //
    ...
    const setInitialPrescriptionDataFromLastPatientPrescription = (
      lastPatientPrescription: any,
    ) => {
      setInitialPrescriptionData({
        user_id: 0,
        patient_id: 0,
      ...
      <div>
                  ...
                  body: (rowData: any) => (
                    <>
                      <Dropdown
                        ...
                        onChange={(e) =>
                          updateDiagnosis(diagnoses.indexOf(rowData), {
                            ...rowData,
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      ...
                      <Dropdown
                        ...
                        placeholder="Seleccione la atención"
                        filter
                      ></Dropdown>
                    </>
                  )}
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      <label
                        ...
                        className="form-label"
                      >
                        Seleccione el codigo de servicio
                      </label>
                      <Dropdown
                        id={field.name}
                        {...field}
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      <label
                        ...
                        className="form-label"
                      >
                        Seleccione el grupo de servicio
                      </label>
                      <Dropdown
                        id={field.name}
                        {...field}
                  ...
                  render={({ field, fieldState }) => (
                    <>
                      ...
                      <Dropdown
                        ...
                        optionValue="code"
                        options={[
                          { name: "Imppresion diagnóstica", code: "1" },
                          { name: "Confirmado nuevo", code: "2" },
                          { name: "Confirmado repetido", code: "3" },
                        ]}
                        placeholder="Seleccione el tipo de diagnóstico principal"
                        filter
                      ></Dropdown>
                    </>
                  )}
        ...
        <div className="d-flex">
          ...
          <div className="p-3 flex-grow-1">
            ...
            >
              ...
              <div className={prescriptionsActive ? "d-block" : "d-none"}>
                <div className="d-flex align-items-center gap-2 mb-3">
                  ...
                  />
                  <label htmlFor="loadLastPrescriptionCheck">
            ...
            <div className={activeTab === "optometry" ? "d-block" : "d-none"}>
              <div className="d-flex justify-content-between">
              ...
              <div className={optometryActive ? "d-block" : "d-none"}>
                <OptometryPrescriptionForm ref={optometryFormRef} />
              </div>
            </div>
            <div
...
const Tab: React.FC<TabProps> = ({
  ...
  return (
    <>
      <Button
        className={`w-100 p-3 p-button-primary ${
          activeTab === tab.key ? "p-button-primary text-white" : ""
        } btn-sm`}
        onClick={() => {
